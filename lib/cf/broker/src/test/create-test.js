...
describe('Create service instance', () => {
  ...
  context('when error', () => {
    ...
    const validateErrorInPlansCreation =
      (callCount, expectedErrorMessage, done) =>
        createService.createPlans(planConfig, (statusCode, errorMessage) => {
    ...
    it('500 is retured during plan creation, it should fail ', (done) => {
      ...
      validateErrorInPlansCreation(1, errorMessage, done);
    });
    ...
    it('404 is retured during plan creation, it should fail', (done) => {
      ...
      validateErrorInPlansCreation(1,
        httpStatus.getStatusText(httpStatus.INTERNAL_SERVER_ERROR), done);
    });
    ...
    it('in first create of plan mapping occurs, it should fail', (done) => {
      ...
      validateErrorInPlansCreation(4,
        httpStatus.getStatusText(httpStatus.INTERNAL_SERVER_ERROR), done);
    });
  });
    ...
    context('plans and mappings are created', () => {
      ...
      const itShouldCreate = (name, callIndex, requestParameters) => {
        it(`${name} should be created`, () => {
          expect(postStub.getCall(callIndex).calledWithMatch(sinon.match.any,
