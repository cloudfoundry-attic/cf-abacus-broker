...
const createPricingPlan = (plans, planId, cb) => {
  ...
  if(isEmpty(plans))
    ...
    pricingPlan = samplePricingPlan(planId,
      config.buildPricingPlanMetrics(metrics));
  }

  createPlan('pricing', pricingPlan, planId, (err, planId) =>
    cb(err, plans, planId));
};
...
const createRatingPlan = (plans, planId, cb) => {
  ...
  if(isEmpty(plans))
    ...
    const metrics = plans[0].plan.metrics;
    ratingPlan = sampleRatingPlan(planId,
      config.buildRatingPlanMetrics(metrics));
  }

  createPlan('rating', ratingPlan, planId, (err, planId) =>
    cb(err, plans, planId));
};
...
const generateErrorMessage = (errorMessage) => {
  ...
  if(errorMessage && errorMessage.statusCode)
    result = httpStatus.getStatusText(httpStatus.INTERNAL_SERVER_ERROR);
  else
    result = errorMessage;
...
const createPlans = (requestConfig, cb) => {
  ...
  async.series([
    ...
    if (err) {
      ...
      cb(httpStatus.INTERNAL_SERVER_ERROR, generateErrorMessage(err));
      return;
    }
