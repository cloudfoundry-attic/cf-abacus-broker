...
// Post resource provider configuration to provided Mapping API
const postMapping = (body, serviceId, instanceId, planId, cb) => {
...
const mappingRequestBody = (planConfig) => {
  return {
...
const getResourceProvider = (plans) =>
  !isEmpty(plans) ? plans[0].resource_provider : undefined;

const validCustomConfig = (parameters) => {
...
const tasks = (planConfig) => {
  ...
  if(resourceProvider)
    tasks = [async.apply(postMapping,
      mappingRequestBody(planConfig), planConfig.service_id,
      planConfig.instance_id, planId), async.apply(createMeteringPlan, plans)]
      .concat(tasks);
  else
    tasks = [async.apply(createMeteringPlan, plans, planId)].concat(tasks);
...
const createPlans = (planConfig, cb) => {
  ...
  if(!validCustomConfig(planConfig.parameters)) {
