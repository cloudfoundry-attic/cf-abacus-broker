...
class CfApiController {
  ...
  getServiceCredentials(request) {
    ...
    return this.httpClient.request(
      helper.generateRequestObject('GET',serviceKeysUrl,
        request.session.uaa_response.access_token)).then((result) => {
    ...
    return this.httpClient.request(
      helper.generateRequestObject('GET', bindingsUrl,
        request.session.uaa_response.access_token)).then((serviceBindings) => {
          ...
          logger.error('Unable to find service-keys or service bindings for this Instance');
          return Promise.reject(new errors.NotFound('Unable to find service-keys or service bindings for this Instance'));
        });
  };
