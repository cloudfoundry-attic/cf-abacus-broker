...
jsdom.env("./test/backend/coverage/index.html", (err, window) => {
    ...
    window.document.documentElement.getElementsByTagName('tbody')[0].appendChild(table);
    fs.writeFile('./test/backend/coverage/index.html', window.document.documentElement.outerHTML, error => {
        if (error) throw error;
    });
    fs.rename('./test/backend/coverage/index.html', './coverage/index.html', (err) => {
        if (err) throw err;
        console.log('Final coverage recorded at /coverage/index.html');
    });
});

let updateStats = (metrics, stats) => {

    for (let cnt = 0; cnt < metrics.length; cnt++) {
        let value = stats[cnt];
        let startCountUI = parseInt(value.substring(value.indexOf("(") + 1, value.indexOf("/") - 1));
        let endCountUI = parseInt(value.substring(value.indexOf("/") + 2, value.indexOf(")")));

        let startCountBackend = parseInt(metrics[cnt].substring(0, metrics[cnt].indexOf("/")));
        let endCountBackend = parseInt(metrics[cnt].substring(metrics[cnt].indexOf("/") + 1));
...
//Clean up code : 
fs.rename('./test/backend/coverage/prettify.css', './coverage/prettify.css', (err) => {
    if (err) throw err;
});
fs.rename('./test/backend/coverage/prettify.js', './coverage/prettify.js', (err) => {
    if (err) throw err;
});
fs.rename('./test/backend/coverage/base.css', './coverage/base.css', (err) => {
    if (err) throw err;
});
fs.rename('./test/backend/coverage/sorter.js', './coverage/sorter.js', (err) => {
    if (err) throw err;
});
