...
class CfApiController {
    ...
    getSharedDomainsCall(request) {
        ...
        if (_.isEmpty(config.cf.abacus_provisioning_endpoint)) {
            return this.httpClient.request(helper.generateRequestObject('GET', sharedDomainsUrl, request.session.uaa_response.access_token)).then((res) => {
                    let domains = res.body;
                    if (domains.resources.length > 0) {
                        let domainName = domains.resources[0].entity.name;
                        let provisioning_plugin = config.cf.abacus_provisioning_plugin;
                        config.cf.abacus_provisioning_endpoint = `https://${provisioning_plugin}.${domainName}`;
                    }
                })
                .catch((error) => {
    ...
    getServiceBinding(request) {
        ...
        let serviceBindingsPromise = this.httpClient.request(helper.generateRequestObject('GET', bindingsUrl, request.session.uaa_response.access_token));
        let sharedDomainsPromise = this.getSharedDomainsCall(request);
        return Promise.join(serviceBindingsPromise, sharedDomainsPromise, (serviceBindings, sharedDomains) => {
            ...
            if (bindings.resources.length > 0) {
                ...
                return authResponse.then(auth_response => {
                    request.session.abacus_token = auth_response.body.access_token;
                });
