...
const meteringLock = yieldable(lockcb);
const ratingLock = yieldable(lockcb);
const pricingLock = yieldable(lockcb);
...
const findPlan = function *(pid, lock, db, cache) {
  ...
  try {
    ...
    if(doc) {
      ...
      const undbified = dbclient.undbify(doc);
  ...
  read: {
    withCache: {
      metering: function *(id) {
        return yield findPlan(id, meteringLock, meteringdb, meteringCache);
      },
      pricing: function *(id) {
        return yield findPlan(id, pricingLock, pricingdb, pricingCache);
      },
      rating: function *(id) {
        return yield findPlan(id, ratingLock, ratingdb, ratingCache);
      }
    },
    withoutCache: {
      metering: function *(id) {
        return yield findPlan(id, meteringLock, meteringdb, voidCache);
      },
      pricing: function *(id) {
        return yield findPlan(id, pricingLock, pricingdb, voidCache);
      },
      rating: function *(id) {
        return yield findPlan(id, ratingLock, ratingdb, voidCache);
      }
    }
  },
