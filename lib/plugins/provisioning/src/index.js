...
const wscope = { system: ['abacus.usage.write'] };
const rscope = { system: ['abacus.usage.read'] };
...
const assureAuthorized = (req, scope) => {
  if (secured())
...
routes.get('/v1/:plan_type/plans/:plan_id',
  function *(req) {
    assureAuthorized(req, rscope);
    debug('Retrieving %s plan %s', req.params.plan_type, req.params.plan_id);
    let cacheOpt = true;
