...
const upsertPlan = function *(type, plan) {
  if (yield plansDb.read.withoutCache[type](plan.plan_id))
...
const createDbReader = (cacheOpt) => {
  if(cacheOpt === 'no-cache')
    return plansDb.read.withoutCache;

  return plansDb.read.withCache;
};
...
routes.get(
  ...
  function *(req) {
    ...
    const mp = yield plansDb.read.withoutCache.metering(id);
    if(!mp)
      return {
        status: 404,
        body: path
...
routes.get(
  '/v1/:plan_type/plans/:plan_id',
  function *(req) {
    debug('Retrieving %s plan %s', req.params.plan_type,
      req.params.plan_id);
    const cacheOpt = req.headers['cache-control'];
    let dbReader = createDbReader(cacheOpt);

    const plan = yield dbReader[req.params.plan_type]
      (req.params.plan_id, cacheOpt === 'no-cache');
