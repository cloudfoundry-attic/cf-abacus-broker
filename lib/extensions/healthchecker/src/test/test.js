...
describe('Abacus healthchecker', () => {
  ...
  const stubRequestGet = (statusCode, error) => {
    if (error) {
  ...
  it('should wrap the error, when request returns an error', (done) => {
    stubRequestGet(undefined, new Error('error'));
  ...
  context('when an app is unhealthy', () => {
    ...
    const stubRequestGetWithArgs = (args) => {
      const getStub = sandbox.stub(request, 'get');
      Object.keys(args).forEach((key) => {
        getStub.withArgs(key).yields(undefined, { statusCode: args[key] });
      });
    };
    ...
    it('returns status code 200 for every app, when APPLICATION_GROUPS ' +
      'have just one instance per group', (done) => {
      stubRequestGetWithArgs({
        'http://aggregator.cfapps.localhost/healthcheck' : 
          httpStatus.OK,
        'http://collector.cfapps.localhost/healthcheck' : 
